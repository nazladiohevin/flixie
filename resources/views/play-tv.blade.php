    ...
    <section>
      <div class="container mx-auto px-6 xl:px-24">      
        <div class="flex flex-wrap gap-10 lg:flex-nowrap">
          <div class="basis-full lg:basis-[70%]">              
            <iframe id="containerVidio" class="w-full h-52 md:h-[500px]" src="https://www.youtube.com/embed/{{ $episode->url_vidio }}" allowfullscreen></iframe>     
          </div>     
          <div class="basis-full lg:basis-[30%] border-2 border-white/80 rounded-sm p-3 sm:p-8">
            <div class="mb-6">
              <select id="seasons" class="bg-neon-300 border border-gray-300 text-black font-bold rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                @foreach ($film->season->unique("season") as $season)
                    <option value="{{ $season->season }}" {{ $season->season == $episode->season ? 'selected' : '' }} class="text-black">Season {{ $season->season }}</option>
                @endforeach
              </select>
            
              {{-- <button data-dropdown-toggle="season-list"  data-dropdown-placement="right" class="font-semibold bg-neon-300 px-3 py-1 sm:px-6 sm:py-2 text-black rounded-[10px]">
                Season 1
                <svg class="inline-block ml-1 size-6 text-black" xmlns="http://www.w3.org/2000/svg"fill="none" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-5 sm:grid-cols-7 lg:grid-cols-3 xl:grid-cols-5 gap-3" id="listEpisode">
              @foreach ($episodes as $item)
                @php
                  $is_episodeActive = $episode->episode == $item->episode  && $episode->season ==$item->season;
                  $is_seasonActive = $episode->season == $item->season;
                @endphp
                <a href="{{ '/films/' . $film->slug . "/" . $item->season . "/" . $item->episode}}" 
                  class="{{ 's' . $item->season }} {{ !$is_seasonActive ? 'hidden' : ''}} {{ $is_episodeActive ? 'bg-lagoon-700' : 'bg-lagoon-900' }} text-h5 text-white/75 font-bold rounded-lg py-2 text-center cursor-pointer hover:bg-lagoon-800">
                  {{ $item->episode }}
                </a>               
              @endforeach
            </div>              
      ...
      <div class="container mx-auto px-6 mt-10 xl:px-24">
        ...
        <div class="flex gap-x-8 gap-y-32 flex-wrap xl:flex-nowrap xl:gap-x-28">
          <div class="basis-full xl:basis-[60%]">
            <h4 class="text-xl font-bold text-white sm:text-h4">{{ $film->title }} > Season {{ $episode->season }} > Episode {{ $episode->episode }}</h4>
...
@endsection
@section("js-custom")
  <script src="{{ asset("js/play-film.js") }}"></script>
@endsection
